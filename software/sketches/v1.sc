...
(
	...
	SynthDef.new("TriOsc", { arg out = 0, freq = 440, gain = 0.5;
		Out.ar(
			out,
			LFTri.ar(
				freq,
	...
	SynthDef.new("SawOsc", { arg out = 0, freq = 440, gain = 0.5;
		Out.ar(
			out,
			Saw.ar(
				freq,
	...
	SynthDef.new("PulseOsc", { arg out = 0, freq = 440, gain = 0.5, width = 0.5;
		Out.ar(
			out,
			Pulse.ar(
				freq,
	...
	SynthDef.new("SinOsc", { arg out = 0, freq = 440, gain = 0.5;
		Out.ar(
			out,
			SinOsc.ar(
				freq,
	...
	SynthDef.new("PrimaryMixer", { arg out = 0, c1, c2 ;
		Out.ar(
			out,
			Mix.new([c1, c2])
		)
	...
	SynthDef.new("Test", {arg out = 0, freq = 440, lfo = 0, gain = 0.5;
		Out.ar(
			out,
			SinOsc.ar(
				freq+lfo,
	...
	~synth = Synth("Test", ["freq", 900]);
	
	~lfo = Synth("lfo", ["out", 2]);
	
	~bus = Bus.control(s, 1).set(600);
	~lfo.set("out", ~bus.index);
	~lfo.set("gain", 10);
	~synth.set("lfo", ~bus.asMap);
